...
def render_view_fallback(camera: Camera, pc: GaussianModel, bg_color: torch.Tensor, override_color=None):
    means3D = pc.get_xyz
    opacity = pc.get_opacity

    if override_color is None:
        shs_view = pc.get_shs.transpose(1, 2).view(-1, 3, (pc.max_sh_degree + 1) ** 2)
        dir_pp = means3D - camera.camera_center.repeat(pc.get_shs.shape[0], 1)
        dir_pp_normalized = F.normalize(dir_pp, dim=-1)
        colors = torch.clamp_min(eval_sh(pc.active_sh_degree, shs_view, dir_pp_normalized) + 0.5, 0.0)
    else:
    ...
    clip = xyz_homo @ camera.full_proj_transform
    ndc = clip[:, :3] / clip[:, 3:].clamp_min(1e-6)
    px = (ndc[:, 0] + 1.0) * 0.5 * camera.image_width
    py = (1.0 - ndc[:, 1]) * 0.5 * camera.image_height
    view = xyz_homo @ camera.world_view_transform
    depths = view[:, 2:3]
    depths2 = depths.square()
    visible = (clip[:, 3] > 0) & (depths[:, 0] > 0)

    fx = camera.intrinsics[0, 0]
    scales = pc.get_scaling.mean(dim=-1, keepdim=True)
    sigma = (scales * fx / depths.clamp_min(1e-4)).clamp(0.75, 3.5)
    visible = visible & (px >= -3.0 * sigma[:, 0]) & (px <= camera.image_width + 3.0 * sigma[:, 0]) \
        & (py >= -3.0 * sigma[:, 0]) & (py <= camera.image_height + 3.0 * sigma[:, 0])

    H, W = camera.image_height, camera.image_width
    yy, xx = torch.meshgrid(
        torch.arange(H, device=means3D.device, dtype=means3D.dtype),
        torch.arange(W, device=means3D.device, dtype=means3D.dtype),
        indexing="ij",
    )
    acc_color = torch.zeros((3, H, W), device=means3D.device, dtype=means3D.dtype)
    acc_normal = torch.zeros((3, H, W), device=means3D.device, dtype=means3D.dtype)
    acc_depth = torch.zeros((1, H, W), device=means3D.device, dtype=means3D.dtype)
    acc_depth2 = torch.zeros((1, H, W), device=means3D.device, dtype=means3D.dtype)
    acc_xyz = torch.zeros((3, H, W), device=means3D.device, dtype=means3D.dtype)
    acc_weight = torch.zeros((1, H, W), device=means3D.device, dtype=means3D.dtype)
    point_weights = torch.zeros((means3D.shape[0], 1), device=means3D.device, dtype=means3D.dtype)
    num_contrib = torch.zeros((H, W), device=means3D.device, dtype=torch.int32)

    chunk_size = 16 if means3D.is_cuda else 128
    for start in range(0, means3D.shape[0], chunk_size):
        end = min(start + chunk_size, means3D.shape[0])
        px_i = px[start:end].view(-1, 1, 1)
        py_i = py[start:end].view(-1, 1, 1)
        sigma_i = sigma[start:end].view(-1, 1, 1)
        opacity_i = opacity[start:end].view(-1, 1, 1)
        vis_i = visible[start:end][:, None, None]

        dx = xx[None, ...] - px_i
        dy = yy[None, ...] - py_i
        d2 = dx.square() + dy.square()
        support = d2 <= (3.0 * sigma_i).square()
        w = torch.exp(-0.5 * d2 / sigma_i.square().clamp_min(1e-6)) * opacity_i * support * vis_i

        acc_weight += w.sum(dim=0, keepdim=True)
        acc_color += (w[:, None] * colors[start:end, :, None, None]).sum(dim=0)
        acc_normal += (w[:, None] * normals[start:end, :, None, None]).sum(dim=0)
        acc_depth += (w * depths[start:end].view(-1, 1, 1)).sum(dim=0, keepdim=True)
        acc_depth2 += (w * depths2[start:end].view(-1, 1, 1)).sum(dim=0, keepdim=True)
        acc_xyz += (w[:, None] * means3D[start:end, :, None, None]).sum(dim=0)
        point_weights[start:end] = w.sum(dim=(1, 2), keepdim=True).view(-1, 1)
        num_contrib += support.sum(dim=0).to(torch.int32)

    norm = acc_weight.clamp_min(1e-6)
    rendered_opacity = (1.0 - torch.exp(-acc_weight)).clamp(0.0, 1.0)
    rendered_image = acc_color / norm
    rendered_image = rendered_image * rendered_opacity + (1.0 - rendered_opacity) * bg_color[:, None, None]
    rendered_normal = F.normalize(acc_normal / norm, dim=0, eps=1e-6)
    rendered_depth = acc_depth / norm
    rendered_var = acc_depth2 / norm - rendered_depth.square()
    rendered_surface_xyz = acc_xyz / norm
    rendered_pseudo_normal = depth2normal(rendered_depth, camera.image_mask.cuda(), camera)
    screenspace_points = torch.stack([px, py, depths[:, 0]], dim=-1)
    if screenspace_points.requires_grad:
        screenspace_points.retain_grad()
    radii = torch.where(visible, sigma[:, 0].ceil().to(torch.int32), torch.zeros_like(sigma[:, 0], dtype=torch.int32))
    ...
    return {
        ...
        "radii": radii,
        "num_rendered": int(visible.sum().item()),
        "num_contrib": num_contrib,
        "opacities": opacity,
        "normals": normals,
        "directions": F.normalize(means3D - camera.camera_center.repeat(pc.get_shs.shape[0], 1), dim=-1),
...
def render_view(camera: Camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, 
    ...
    if os.environ.get("SVGIR_USE_PYTORCH_FALLBACK") == "1":
        return render_view_fallback(camera, pc, bg_color, override_color)
