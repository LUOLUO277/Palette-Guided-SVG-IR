...
try:
    from simple_knn._C import distCUDA2 as _distCUDA2
    _simple_knn_import_error = None
except Exception as exc:
    _distCUDA2 = None
    _simple_knn_import_error = exc
...
def distCUDA2(points: torch.Tensor) -> torch.Tensor:
    if _distCUDA2 is not None:
        return _distCUDA2(points)
    ...
    chunk_size = 256 if points.is_cuda else 1024
    ...
    for start in range(0, num_points, chunk_size):
        ...
        distances[row_indices, start + row_indices] = float("inf")
        min_distances[start:end] = distances.min(dim=1).values

    return min_distances.square()
...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int, render_type='render'):
        ...
        if self.use_pbr:
            ...
            if Renderer is None:
                try:
                    Renderer = importlib.import_module("pbgi.renderer")
                except Exception as exc:
                    raise ImportError(
            ...
            if RayTracer is None:
                try:
                    RayTracer = importlib.import_module("submodules.bvh").RayTracer
                except Exception as exc:
                    raise ImportError(
                        "PBR rendering requires the BVH extension, which is not available."
                    ) from exc
            if Renderer is None:
                raise ImportError(
                    "PBR rendering requires pbgi/slangtorch dependencies that are not available."
