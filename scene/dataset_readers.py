...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, debug=False):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        mask = np.ones([image.size[1], image.size[0], 1]).astype(np.float32)
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", debug=False):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(tqdm(frames, leave=False)):
            ...
            try:
                monoN_path = image_path.replace(Path(image_path).name, 
                                                Path(image_path).name.replace(image_name, 'normal').replace(Path(image_path).suffix, '.npy'))
            ...
            if mono is None:
                try:
                    monoN_path = image_path.replace(Path(image_path).name, 
                                                    Path(image_path).name.replace(image_name, image_name+'_normal').replace(Path(image_path).suffix, '.npy'))
                    monoN = read_monoData(monoN_path)
                    monoD = np.zeros_like(monoN[:1])
                    mono = np.concatenate([monoN, monoD], 0)
                except:
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=fovy, FovX=fovx, image=image, image_mask=image_mask,
                                        ...
                                        width=image.shape[1], height=image.shape[0], mono=mono))
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png", debug=False):
    ...
    if not os.path.exists(ply_path):
        ...
        num_pts = int(os.environ.get("SVGIR_SMOKE_NUM_PTS", "100000"))
...
def readrender_relightInfo(path, white_background, eval, debug=False):
    validation_indexes = []
    # if "data_dtu" in path:
    if True:
        if eval:
...
def readCamerasFromTransforms3(path, transformsfile, white_background, extension=".png", debug=False):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(tqdm(frames, leave=False)):
            ...
            try:
                monoN_path = image_path.replace(Path(image_path).name, 
                                                Path(image_path).name.replace("rgb", 'normal').replace(Path(image_path).suffix, '.npy'))
                monoN = read_monoData(monoN_path)
                monoD = np.zeros_like(monoN[:1])
                mono = np.concatenate([monoN, monoD], 0)
            except:
...
sceneLoadTypeCallbacks = {
    ...
    "render_relight": readrender_relightInfo,
